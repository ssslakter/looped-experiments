...
# %% auto 0
__all__ = ['liner_clm', 'log_clm', 'sqrt_clm', 'cos_clm', 'Curriculum', 'CurriculumCB', 'LoopCB']
...
def liner_clm(cfg, steps):
    return torch.minimum(torch.tensor(cfg.end), cfg.start + steps // cfg.interval * cfg.inc)


def log_clm(cfg, steps):
    return torch.round((cfg.end - cfg.start) / math.log(1 + len(steps)) * torch.log1p(steps) + cfg.start)
...
class Curriculum:
    ...
    def __init__(self, cfg, n_steps=10_000, clm_type='linear', clm_func=None):
        ...
        self.values = f(cfg, self.steps).type(torch.int32)
...
class CurriculumCB(Callback):
    def __init__(self, curriculum_config, train_config):
        ...
        self.curs = {k: Curriculum(v, n_steps=train_config.train_steps * train_config.n_epoch, clm_type=v.type)
